...
export default function ShareModal({ trip, userId, userEmail, onClose }: {
  ...
  const copyInviteLink = async (inviteEmail: string) => {
    const link = `${window.location.origin}/invite?trip=${trip.id}`
    await navigator.clipboard.writeText(
      `You've been invited to collaborate on "${trip.name}" in Travel Planner.\n\nClick here to join: ${link}\n\nSign in with: ${inviteEmail}`
    )
    setCopiedId(inviteEmail)
                ...
                <div className="space-y-2">
                  {pending.map((p) => (
                    <div key={p.id} className="flex items-center gap-3">
                      ...
                      <button onClick={() => handleRemoveInvite(p.id)}
                        className="text-gray-300 hover:text-red-400 transition-colors">
                        <Icon name="close" />
                      </button>
                    </div>
