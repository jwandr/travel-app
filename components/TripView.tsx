...
import {
  ...
  DragOverlay, useDroppable, type DragEndEvent, type DragStartEvent, type DragOverEvent,
} from '@dnd-kit/core'
import {
          ...
          {datesChanged && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
              <p className="text-xs text-amber-700">
                ⚠️ Changing dates will update day dates. Items on days beyond the new duration will be removed.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900">
            {accom ? 'Edit Accommodation' : 'Add Accommodation'}
          </h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl leading-none">✕</button>
        </div>
        <div className="space-y-3">
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} autoFocus
              placeholder="e.g. Hotel Kanra Kyoto"
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" />
          </div>
          <div>
          ...
          <div className="flex gap-2">
            <div className="flex-1">
              ...
              <input type="date" value={checkIn} min={tripStart} max={tripEnd}
                onChange={(e) => setCheckIn(e.target.value)}
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400" />
            </div>
            <div className="flex-1">
              <label className="block text-sm font-medium text-gray-700 mb-1">Check-out</label>
              <input type="date" value={checkOut} min={tripStart} max={tripEnd}
              ...
              <div className="flex flex-wrap gap-1.5">
                {(Object.keys(TYPE_CONFIG) as ItemType[]).map((t) => (
                  <button key={t}
                    ...
                    className={`text-xs px-2.5 py-1 rounded-full font-medium transition-opacity flex items-center gap-1 ${
                      TYPE_CONFIG[t].bg} ${TYPE_CONFIG[t].text
                    } ${item.type === t ? 'opacity-100' : 'opacity-30 hover:opacity-60'}`}>
      ...
      <div className="flex items-center gap-3 flex-1 min-w-0 p-3">
        ...
        <div className="shrink-0">
          <div className={`w-2 h-2 rounded-full ${
            item.start_time ? cfg.bg.replace('100', '400') : 'bg-gray-200'
          }`} />
        </div>
      ...
      <SortableContext items={items.map((i) => i.id)} strategy={verticalListSortingStrategy}>
        <div ref={setNodeRef} className={`space-y-2 min-h-[56px] rounded-xl transition-colors ${
          ...
          {items.length === 0 && isOver && (
            <div className="flex items-center justify-center h-14 text-xs text-indigo-400 font-medium">
              Drop here
            </div>
          )}
          {items.map((item) => (
    ...
    <div className="border-t border-gray-100 bg-white px-6 py-3 flex items-center gap-8 shrink-0">
      {stats.map((s) => (
        <div key={s.label} className="flex items-center gap-2.5">
          <Icon name={s.icon} className="text-indigo-400 !text-xl" />
          <div>
            <div className="text-base font-bold text-gray-900 leading-none">{s.value}</div>
...
export default function TripView({ trip: initialTrip, days: initialDays, userId }: {
  ...
  const [accomModal, setAccomModal] = useState<{ open: boolean; accom?: Accommodation }>({ open: false })
...
{/* Day tabs + accommodation strip — single scrolling container */}
          <div className="mt-4 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
            <div className="flex flex-col" style={{ width: 'max-content' }}>
              ...
              <div className="flex gap-1">
                {/* Overview tab */}
                <button
                  onClick={() => setActiveTabId(OVERVIEW_ID)}
                  className={`flex flex-col items-center px-4 py-2 rounded-xl text-xs shrink-0 transition-colors border ${
                    isOverview
                  ...
                  style={{ minWidth: '96px' }}
                >
                  <span className={`font-semibold text-sm ${isOverview ? 'text-indigo-700' : 'text-gray-700'}`}>
                    Overview
                  </span>
                  <span>All days</span>
                </button>
                ...
                {days.map((day) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center px-4 py-2 rounded-xl text-xs shrink-0 transition-colors border relative ${
                        active
                          ? 'border-indigo-200 bg-indigo-50 text-indigo-700'
                          : 'border-transparent text-gray-400 hover:bg-gray-50'
                      }`}
              ...
              <>
                ...
                {activeDay && (
                  <p className="text-sm text-gray-400 mb-4 max-w-2xl">
                    {new Date(activeDay.date).toLocaleDateString('en-AU', {
                ...
                <DndContext
                  ...
                  collisionDetection={closestCenter}
                  onDragStart={handleDragStart}
                  onDragEnd={handleSingleDayDragEnd}
                >
                    ...
                    <div className="space-y-2 max-w-2xl">
                      {singleDayItems.length === 0 && (
                        <div className="text-center py-16 text-gray-300 text-sm">
                          Nothing planned yet — add something below.
                        </div>
                      )}
      ...
      {/* Modals */}
      {showEdit && (
        <EditTripModal trip={trip} onSaved={handleTripSaved} onClose={() => setShowEdit(false)} />
      )}
