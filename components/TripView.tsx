...
function AccommodationCard({
  ...
  return (
    <div
      onClick={onClick}
      className="flex items-center rounded-xl border border-sky-100 bg-white hover:border-sky-200 hover:shadow-sm transition-all overflow-hidden cursor-pointer"
    >
      <div className="flex items-center gap-3 flex-1 min-w-0 p-3">
        <div className="w-9 h-9 shrink-0 rounded-xl flex items-center justify-center bg-sky-100">
          <Icon name="bed" className="text-sky-600" />
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            <span className="text-xs text-gray-400">
              {new Date(accom.check_in).toLocaleDateString('en-AU', {
                day: 'numeric',
                month: 'short',
              })}
              {' → '}
              {new Date(accom.check_out).toLocaleDateString('en-AU', {
                day: 'numeric',
                month: 'short',
              })}
            </span>
            ...
            {accom.confirmed && (
              <span className="inline-flex text-xs px-2 py-0.5 bg-green-50 text-green-600 border border-green-200 rounded-full font-medium">
                Confirmed
              </span>
            )}
          </div>
          ...
          {accom.confirmation && (
            <div className="text-xs text-sky-500 mt-1 truncate">
              {accom.confirmation}
            </div>
          )}
        </div>
