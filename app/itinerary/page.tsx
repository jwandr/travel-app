...
import {
  fetchItinerary,
  saveLeg,
  deleteLeg as dbDeleteLeg,
  deleteActivity as dbDeleteActivity,
  deleteAccomNote as dbDeleteAccomNote,
  updateItinerary,
} from '@/lib/itinerary'
import type {
  Itinerary,
...
function computeStartDates(legs: ItineraryLeg[], itineraryStart: string): Map<string, Date> {
  const map = new Map<string, Date>()
  let cursor = new Date(itineraryStart + 'T00:00:00')
  for (const leg of legs) {
    map.set(leg.id, new Date(cursor))
    cursor = addDays(cursor, leg.duration_days)
  }
  return map
}
...
const MODE_STYLES: Record<ItineraryMode, { bg: string; text: string; icon: string }> = {
  ...
  Reset:      { bg: 'bg-green-50',  text: 'text-green-700',  icon: 'self_improvement' },
}
...
function Icon({ name, className = '' }: { name: string; className?: string }) {
  return (
    <span className={`material-symbols-rounded ${className}`} style={{ fontSize: 18 }}>
      {name}
    </span>
  )
}
...
function ModeBadge({ mode }: { mode: ItineraryMode }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${s.bg} ${s.text}`}>
      <Icon name={s.icon} className={`${s.text} !text-sm`} />
      {mode}
    </span>
  )
}
...
function ItineraryModal({
  ...
  onSelect, onCreate, onUpdate, onDelete, onClose,
}: {
  ...
  const openEdit = (itin: ItineraryRow) => {
    setEditTarget(itin); setTitle(itin.title)
    setStartDate(itin.start_date); setYearLabel(itin.year_label ?? '')
    setMode('edit')
  }
  const handleSubmit = () => {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="font-semibold text-gray-900 text-sm">
            {mode === 'list' ? 'Itineraries' : mode === 'new' ? 'New itinerary' : 'Edit itinerary'}
          </h2>
              ...
              <div key={itin.id} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors ${itin.id === activeId ? 'bg-sky-50' : 'hover:bg-gray-50'}`}>
                <button className="flex-1 text-left" onClick={() => { onSelect(itin.id); onClose() }}>
                  ...
                  <div className="text-xs text-gray-400 mt-0.5">
                    {itin.year_label && `${itin.year_label} · `}
                    {itin.start_date ? `Starts ${new Date(itin.start_date + 'T00:00:00').toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' })}` : ''}
                  </div>
                </button>
                ...
                {itineraries.length > 1 && (
                  <button onClick={() => { if (confirm('Delete this itinerary and all its legs?')) { onDelete(itin.id); onClose() } }} className="text-gray-300 hover:text-rose-400 p-1">
                    <Icon name="delete" className="!text-base" />
                  </button>
                )}
              </div>
            ))}
            <button onClick={openNew} className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm text-sky-600 hover:bg-sky-50 transition-colors">
          ...
          <div className="p-5 space-y-4">
            <div>
              ...
              <input type="text" value={title} onChange={e => setTitle(e.target.value)} autoFocus
                ...
                placeholder="e.g. Round the World" />
            </div>
            ...
            <div className="flex gap-2 pt-1">
              ...
              <button onClick={handleSubmit} disabled={!title.trim() || !startDate}
                className="flex-1 px-3 py-2 bg-sky-600 text-white rounded-xl text-sm font-medium hover:bg-sky-700 disabled:opacity-40 disabled:cursor-not-allowed">
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-3">
        {inp('From', 'from_airport', 'PER')}
        {inp('To', 'to_airport', 'SCL')}
        {inp('Via (optional)', 'via_airport', 'SYD')}
...
function ActivitiesPanel({ activities, onChange, onDelete }: {
  ...
  const add = () => onChange([...activities, { id: uid(), sort_order: activities.length, description: '', tier: 'nice', category: '' }])
    ...
    <div className="space-y-2">
      ...
      {activities.map(a => (
        <div key={a.id} className="flex items-center gap-2">
          ...
          <input type="text" value={a.category} placeholder="Tag"
            onChange={e => update(a.id, { category: e.target.value })}
            className="w-20 text-sm border border-gray-200 rounded-lg px-2 py-1.5 bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-sky-200" />
...
function AccomPanel({ accom, onChange, onDelete }: {
  accom: AccomNote[]; onChange: (a: AccomNote[]) => void; onDelete: (id: string) => void
}) {
  const add = () => onChange([...accom, { id: uid(), sort_order: accom.length, accom_type: 'TBD', name: '', notes: '', booking_status: 'unplanned' }])
...
function LegRow({
  leg, startDate, expanded, saving,
  onSave, onToggle, onDelete,
  onDeleteActivity, onDeleteAccom,
}: {
  leg: ItineraryLeg
  startDate: Date
  expanded: boolean
  saving: boolean
  onSave: (l: ItineraryLeg) => void
  onToggle: () => void
  onDelete: () => void
  onDeleteActivity: (id: string) => void
  onDeleteAccom: (id: string) => void
}) {
  ...
  const mustActivities = leg.activities.filter(a => a.tier === 'must')
  const costDisplay = isTransit && draft.transit?.cost_aud
    ? `$${Number(draft.transit.cost_aud).toLocaleString('en-AU')}` : null
  ...
  return (
    <div ref={setNodeRef} style={style} className={`border rounded-xl overflow-hidden bg-white transition-shadow ${
      isDragging ? 'shadow-lg border-sky-200' : expanded ? 'border-sky-200 shadow-sm' : 'border-gray-100'
    }`}>
      ...
      <div className="flex items-start gap-2 px-3 py-3">
        ...
        <button
          ...
          {...attributes}
          {...listeners}
          className="shrink-0 mt-1 text-gray-200 hover:text-gray-400 cursor-grab active:cursor-grabbing touch-none transition-colors"
        ...
        {/* Middle: all activities sorted must → nice → optional */}
        <div className="flex-1 min-w-0 self-center">
          {(() => {
            const TIER_ORDER: Record<string, number> = { must: 0, nice: 1, optional: 2 }
            ...
            const sorted = [...leg.activities].sort(
              (a, b) => (TIER_ORDER[a.tier] ?? 9) - (TIER_ORDER[b.tier] ?? 9)
            )
            if (sorted.length === 0) {
              return <span className="text-xs text-gray-300 italic">No activities yet</span>
            }
            return (
              <div className="flex flex-wrap gap-1">
                ...
                {sorted.length > 6 && (
                  <span className="text-xs text-gray-400 self-center">+{sorted.length - 6}</span>
                )}
              </div>
            )
          })()}
        ...
        {/* Edit / Save */}
        <button
          onClick={expanded ? handleSave : onToggle}
          className={`shrink-0 flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium transition-colors ${
            expanded ? 'bg-sky-600 text-white hover:bg-sky-700' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50'
          }`}
        >
      ...
      {expanded && (
        <div className="border-t border-gray-100 bg-gray-50 px-4 py-4 space-y-5">
          ...
          {draft.mode === 'Transit' && (
            <TransitPanel
              detail={draft.transit ?? { id: '', ...emptyTransit }}
              onChange={transit => setDraft(d => ({ ...d, transit }))}
            />
          )}
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            ...
            <AccomPanel
              accom={draft.accom}
              onChange={accom => setDraft(d => ({ ...d, accom }))}
              onDelete={onDeleteAccom}
            />
          </div>

          <div className="flex items-center justify-between pt-1">
            <button onClick={onDelete} className="text-xs text-gray-400 hover:text-rose-500 flex items-center gap-1 transition-colors">
...
function StatsBar({ legs }: { legs: ItineraryLeg[] }) {
  const totalDays = legs.reduce((s, l) => s + l.duration_days, 0)
  const totalCost = legs.reduce((s, l) => s + (Number(l.transit?.cost_aud) || 0), 0)
  ...
  return (
    <div className="flex gap-3 flex-wrap">
      {stat('Regions', regions.toString())}
      {stat('Destinations', dests.toString())}
      {stat('Days planned', totalDays.toString())}
      {stat('Transit cost', `$${totalCost.toLocaleString('en-AU')}`)}
    </div>
  )
}
...
function RegionHeader({ label, legs, startDates }: { label: string; legs: ItineraryLeg[]; startDates: Map<string, Date> }) {
  ...
  return (
    <div className="flex items-center gap-3 pt-3 pb-1">
      ...
      {first && endDate && (
        <span className="text-xs text-gray-400 whitespace-nowrap">{fmtDate(first)} – {fmtDate(endDate)} · {totalDays}d</span>
      )}
...
export default function ItineraryPage() {
  ...
  // dnd-kit sensors — 8px activation distance prevents accidental drags on click
  ...
  const handleCreateItinerary = async (title: string, startDate: string, yearLabel: string) => {
    ...
    const { data, error } = await supabase
      .from('itineraries').insert({ user_id: user.id, title, start_date: startDate, year_label: yearLabel })
      .select().single()
  ...
  const handleAddLeg = async () => {
    ...
    try {
      ...
      const newLeg: ItineraryLeg = {
        ...
        duration_days: row.duration_days, notes: '', transit: null, activities: [], accom: [],
      }
  ...
  // Uses individual UPDATE calls (not upsert) to satisfy RLS policy.
  ...
  const handleDragEnd = useCallback(async (event: DragEndEvent) => {
    ...
    setLegs(prev => {
      ...
      // Persist: individual UPDATE per leg (upsert violates RLS insert policy)
      Promise.all(
        reordered.map(l =>
          supabase
            .from('itinerary_legs')
  ...
  const regions = useMemo(
    () => ['All', ...Array.from(new Set(legs.map(l => l.region)))],
    [legs]
  )
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-4">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <button onClick={() => setShowItinModal(true)}
                className="flex items-center gap-1.5 mt-1 text-sm text-gray-400 hover:text-sky-600 transition-colors group">
                <span>{itinerary?.title ?? '—'}</span>
                {itinerary?.year_label && <><span className="text-gray-300">·</span><span>{itinerary.year_label}</span></>}
                <Icon name="unfold_more" className="!text-base text-gray-300 group-hover:text-sky-500" />
              </button>
            </div>
            <button onClick={handleAddLeg}
              className="flex items-center gap-1.5 px-3 py-2 bg-sky-600 text-white text-sm font-medium rounded-xl hover:bg-sky-700 transition-colors shrink-0">
            ...
            {regions.map(r => (
              <button key={r} onClick={() => setFilterRegion(r)}
                className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${
                  filterRegion === r ? 'bg-sky-600 text-white' : 'bg-white border border-gray-200 text-gray-500 hover:border-gray-300'
                }`}>{r}</button>
            ))}
          </div>
          ...
          {/* DnD context wraps the entire list */}
          ...
          <p className="text-xs text-gray-400 text-center pb-4">
            Drag to reorder · Edit to modify · Save to confirm · Dates cascade automatically
          </p>
        </div>
